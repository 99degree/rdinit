...
int main(int argc, char **argv) {
    // Debug bypass: if --debug is first argument, run rdinit_main regardless of PID
    if (argc >= 2 && strcmp(argv[1], "--debug") == 0) {
        LOG_INFOF("Debug mode: forcing rdinit_main even though pid=%d", getpid());
        return rdinit_main();
    }
    ...
    if (strcmp(base, "spawn") == 0 || strcmp(cmd, "spawn") == 0) {
        return spawn_main(argc-2, &argv[2]);
    } else if (strcmp(base, "sudo") == 0 || strcmp(cmd, "sudo") == 0) {
        return sudo_main(argc-2, &argv[2]);
    } else if (strcmp(base, "chroot") == 0 || strcmp(cmd, "chroot") == 0) {
        return chroot_main(argc-2, &argv[2]);
    } else {
